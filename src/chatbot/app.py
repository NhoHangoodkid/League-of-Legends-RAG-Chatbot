...
st.set_page_config(
    page_title = "Gemini LoL Assistant",
    layout = "wide",
...
try:
    with st.spinner("Connecting to League of Legends Knowledge System (Graph + Vector)..."):
        bot = load_bot()
...
if "messages" not in st.session_state:
    if db_online and st.session_state.current_session_id:
...
with st.sidebar:
    ...
    if db_online:
        ...
        st.warning("PostgreSQL disconnected. Session stored in local memory.")
...
# 1. EMPTY STATE HERO (When no messages yet in current session)
if len(st.session_state.messages) == 0 and not prompt_to_send:
    st.markdown(
        """
        <div class='gemini-hero'>
            <h1 class='gemini-title'>Hello, Summoner</h1>
            <p class='gemini-subtitle'>How can I assist you on the Rift today?</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
user_input = st.chat_input("Ask anything about Champions, Items, Counters, Builds, Runes, Lore...")
if prompt_to_send:
...
if user_input and bot:
    ...
    with st.chat_message("assistant", avatar = None):
        ...
        with st.spinner("Consulting League of Legends tactical knowledge base..."):
            t_start = time.time()
        ...
        if isinstance(raw_response, str) and raw_response.strip().startswith("{") and "structured_data" in raw_response:
            try:
                ...
                s_data = parsed.get("structured_data", {})
                champ = s_data.get("champion", "")

                lines = [f"### Tactical Analysis: **{champ or user_input}**\n"]
                if "weak_against" in s_data:
                    lines.append("#### Weak Against:")
                    for c in s_data["weak_against"]:
                        lines.append(f"- **{c.get('champion')}** (Win Rate: {c.get('winRate', '?')}%) — *{c.get('reason', '')}*")
                if "strong_against" in s_data:
                    lines.append("\n#### Strong Against:")
                    for c in s_data["strong_against"]:
                        lines.append(f"- **{c.get('champion')}** (Win Rate: {c.get('winRate', '?')}%) — *{c.get('reason', '')}*")
                if "core_items" in s_data or "coreItems" in s_data:
                    items = s_data.get("core_items") or s_data.get("coreItems")
                    lines.append(f"\n#### Core Items: **{', '.join(items)}**")
                if "full_build" in s_data or "fullBuild" in s_data:
                    fitems = s_data.get("full_build") or s_data.get("fullBuild")
                    lines.append(f"- **Full Build Finisher**: {', '.join(fitems)}")
                if "keystone" in s_data:
                    lines.append(f"- **Recommended Keystone**: `{s_data.get('keystone')}`")
...
st.markdown(
    "<div class='chat-disclaimer'>LoL RAG AI may produce inaccurate tactical advice. Please verify critical details before entering a match.</div>",
    unsafe_allow_html=True,
)
