...
6. Relationship Generator: Generates counters, synergies, builds, and syncs KB data to src/data/knowledge_base/

Usage:
    python src/processors/main.py                 # Run all processors (including KB generation)
    ...
    python src/processors/main.py --relationships # Only generate KB relationships (counters, synergies, builds)
...
def run_all_processors():
    ...
    # 1. Merge Champions
    print("\n[1/6] Merging Champion Data (DDragon + CDragon + Meraki + Lore)")
    t0 = time.time()
    cm = ChampionMerger()
    ...
    # 2. Merge Items
    print("\n[2/6] Merging Item Data (DDragon + CDragon)")
    t0 = time.time()
    im = ItemMerger()
    ...
    print("\n[4/6] Analyzing Champion Spells (CC Types & Ability Effects)")
    t0 = time.time()
    sa = SpellAnalyzer()
    ...
    print("\n[5/6] Enriching Champions with Strategic Metadata")
    t0 = time.time()
    en = Enricher()
    results["enriched_champions"] = en.enrich()
    print(f"-> Enrichment finished in {time.time() - t0:.2f}s")

    # 6. Relationship & Knowledge Base Generator (Counters, Synergies, Builds, KB Sync)
    print("\n[6/6] Generating Knowledge Base Relationships (Counters, Synergies, Builds)")
    t0 = time.time()
    rg = RelationshipGenerator()
    results["kb"] = rg.generate()
    kb_res = results["kb"]
    print(f"-> Knowledge Base relationships generated in {time.time() - t0:.2f}s")
    ...
    print(f"Knowledge Base saved to: {rg.kb_dir}")
    print(f"  - counters/      : {kb_res.get('counters', 0)} files")
    print(f"  - synergies/     : {kb_res.get('synergies', 0)} files")
    print(f"  - builds/        : {kb_res.get('builds', 0)} files")
    print(f"  - champions/     : {kb_res.get('champions', 0)} files")
    print("=" * 60)
...
def main():
    ...
    parser.add_argument(
        ...
        dest="relationships",
        action="store_true",
        help="Run relationship and knowledge base generator only",
    )
    ...
    run_all = args.all or not (
        args.champions or args.items or args.runes or args.spells or args.enrich or args.relationships
    )
    ...
    if args.relationships:
        print("\n[Processor] Running Relationship & Knowledge Base Generator...")
        rg = RelationshipGenerator()
        rg.generate()
