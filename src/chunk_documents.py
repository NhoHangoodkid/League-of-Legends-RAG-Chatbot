"""
Token-aware document chunker for the RAG pipeline.
...
def generate_chunk_id(text: str, doc_path: str = "") -> str:
    """Generate a deterministic short hash ID for a chunk."""
    key = f"{doc_path}::{text.strip()}"
    return hashlib.md5(key.encode("utf-8")).hexdigest()[:12]


def force_split_chunk(text: str, max_tokens: int, overlap_tokens: int) -> List[str]:
    """
...
def split_table_chunk(table_text: str, max_tokens: int) -> List[str]:
    """
...
def merge_small_chunks(chunks: List[Dict[str, Any]], min_tokens: int, hard_cap: int) -> List[Dict[str, Any]]:
    """
...
def infer_domain(doc_path: str) -> str:
...
def chunk_document(data: Dict[str, Any], config: Optional[Dict[str, int]] = None) -> List[Dict[str, Any]]:
    """
