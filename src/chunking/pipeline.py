"""
pipeline.py - Pipeline orchestrator for the RAG preprocessing and chunking pipeline.
...
def print_stats(doc_path,
    ...
    print(f"Blocks: {len(blocks)} ", end="")
    print(" ".join(f"{bt}={cnt}" for bt, cnt in block_types.most_common(5)))

    print(f"Leaf chunks: {n}")
    print(f"Parent chunks: {len(parent_chunks)}")
    print(f"Token distribution: min={token_counts[0]} median={token_counts[n // 2]} max={token_counts[-1]} mean={sum(token_counts) // n}")
...
def run_pipeline(input_path,
    ...
    print(f"Config: hard_cap={chunk_config['hard_cap']} soft_target={chunk_config['soft_target']} overlap={chunk_config['overlap']} min_chunk={chunk_config['min_chunk']}")
    summaries = []
